...
import { usePlayerStore } from "../stores/usePlayerStore" // <--- Import du store
...
function Artists() {
  const { play } = usePlayerStore(); // <--- On récupère la fonction play
        ...
        {artists.map((artist) => (
          <div key={artist.id} className="group relative flex flex-col gap-3 cursor-pointer p-4 rounded-xl hover:bg-white/5 transition-colors duration-300">
            ...
            <div className="relative aspect-square overflow-hidden rounded-full shadow-lg group-hover:shadow-primary/20 transition-all duration-300">
              ...
              {/* Le bouton Play lance maintenant l'action ! */}
              <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                <button 
                  onClick={(e) => {
                    e.stopPropagation(); // Pour éviter de déclencher le clic sur toute la carte
                    play({ title: "Mix Session 2024", artist: artist.name, image: artist.image });
                  }}
                  className="h-12 w-12 rounded-full bg-primary text-white flex items-center justify-center shadow-lg transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300 hover:scale-110"
                >
