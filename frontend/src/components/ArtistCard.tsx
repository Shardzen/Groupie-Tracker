...
export default function ArtistCard({ id, name, image, category }: ArtistCardProps) {
  ...
  const handleClick = () => {
    // 1. On lance la musique (simulée ici "Dernier Mix")
    play({
      title: `Mix de ${name}`,
      artist: name,
      image: image,
    ...
    // 2. On navigue vers la page détail
    navigate(`/artist/${id}`);
  };
  ...
  return (
    <div 
      onClick={handleClick}
      className="group relative flex flex-col gap-3 cursor-pointer p-4 rounded-3xl hover:bg-white/5 transition-colors duration-300"
    >
      ...
      <div className="relative aspect-square overflow-hidden rounded-full shadow-lg group-hover:shadow-[0_0_30px_rgba(124,58,237,0.3)] transition-all duration-500">
        ...
        {/* Overlay Noir au survol (Discret) */}
